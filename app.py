...
# json ausgabe der events des aktuellen Monats (fcellamaro)
@app.route("/<calendar_id>/json/upcommingtasks/")
def jsonRest(calendar_id: str) -> Response:
    ...
    tasksNextMonth = calendar_data.tasks_from_calendar(year=nextYear, month=nextMonth, view_past_tasks=view_past_tasks, data=data)
    tasksNextMonth = calendar_data.add_repetitive_tasks_from_calendar(year=nextYear, month=nextMonth, data=data, tasks=tasksNextMonth,
                                                             view_past_tasks=view_past_tasks)
...
# use to create new password hash (fcellamaro)
#@app.route("/create_hash/<password>")
#def create_hash(password) -> Response:
#  return authentication.create_hash(password);
